...
const KeyboardComponent = (props) => {
  ...
  const createKeyboard = (keys) => {
    ...
    return keys.map((key, index) => {
      if (key === "Enter") {
        return createButton(index,key,"btn-default btn-wide","Submit");
      }
	  else if (key === "Del"){
		return createButton(index,key,"btn-default btn-wide","Delete")
	  }
	  else if (key === " "){
		return <div key={index} className="col p-0 m-1"></div>
	  }
	  else
		return createButton(index,key,"btn-default",key)
    });
  };

  function createButton(index,key,className,value) {
    return (
